...
const resolveUrl = (url: string | URL) => (typeof url === 'string' ? url : url.toString());

function IconLink(rel: string, icon: Icon) {
  if (typeof icon === 'object' && !(icon instanceof URL)) {
    const { url, rel: _, ...props } = icon;
    return `<link rel="${esc(rel)}" href="${esc(resolveUrl(url))}"${Object.keys(props)
  ...
  if (meta.itunes) {
    str += Meta(
      ...
      `app-id=${meta.itunes.appId}${
        meta.itunes.appArgument ? `, app-argument=${meta.itunes.appArgument}` : ''
      }`
    );
  }
